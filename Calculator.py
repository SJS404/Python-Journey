...
from colorama import Fore, init, Back, Style #Call built in colorama module to add colour/style to the output.

init() # Initiates the Colorama Module
num1 = ()
num2 = ()
operator = ()

print(Back.WHITE, Fore.BLUE, Style.BRIGHT,"Welcome to the Calculator!", Style.RESET_ALL, "\n")  # prints welcome message for the user  
print(Style.BRIGHT, "\bStart by entering two numbers, and then choosing the mathematical operator you would like to perform.", Style.RESET_ALL)
...
while True: #Using a while loop, i can validate that the user input is as needed and produce an error message if it is not.
    try:              
      ...
      print(Fore.RED, "\n\bSorry, that is not a valid entry. Please Try Again", Fore.RESET)   # if data is not an int or float, this error will show and loop back to start of the block.
      time.sleep(1)      
      continue
    try:
      ...
      time.sleep(1)      
      continue
          # Dependant on the operator symbol inputted by the user, the calculation will be performed and printed out using the correct mathematical operation. The elif (else if statement) evaluates which operator has been inputted. If it does not match, it move to the next until it matches the required input.
